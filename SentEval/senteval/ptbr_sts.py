...
class PtBrSTSEval(object):
    def __init__(self, task_path, seed=1111):
        logging.debug('***** Transfer task : PtBR-STS*****\n\n')
        self.seed = seed
        train = self.loadFile(os.path.join(task_path, 'valid_sts.txt'))
        dev = self.loadFile(os.path.join(task_path, 'valid_sts.txt'))
        test = self.loadFile(os.path.join(task_path, 'test_sts.txt'))
